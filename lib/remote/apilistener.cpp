	...
	for (;;) {
		...
		for (int ts : files) {
			...
			while (true) {
				...
				try  {
					size_t bytesSend = NetString::WriteStringToStream(client->GetStream(), pmessage->Get("message"));
					endpoint->AddMessageSent(bytesSend);
					count++;
				} catch (const std::exception& ex) {
					Log(LogWarning, "ApiListener")
				...
				if (ts > logpos_ts + 10) {
					...
					size_t bytesSend = JsonRpc::SendMessage(client->GetStream(), lmessage);
					endpoint->AddMessageSent(bytesSend);
				}
			}
